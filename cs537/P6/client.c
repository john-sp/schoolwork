...
#define PRINTV(...) if (verbose) printf("Client: "); if (verbose) printf(__VA_ARGS__)
...
void fork_server() {
	...
	else if (pid > 0) { /* The parent process if there was no error */
		child_pid = pid;
	} else { /* The parent process in case of an error with fork */
		perror("fork");
	}
}

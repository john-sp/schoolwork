...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      for (int i = 0; i < NPROC; i++)
      {
        index = (startIndex + i) % NPROC;
        p1 = &(ptable.proc[index]);
        ...
        //     strncmp(p->name, "sh", sizeof("sh")) != 0)
          // cprintf("Checking %s with priority %d\n", p1->name, p1->elevated);
        if (highP->elevated > p1->elevated) // higher elevated, lower priority
        {
          // cprintf("\nSwitching to higher priority \"%s\" with priority %d vs %s's %d\n", p1->name, p1->elevated, highP->name, highP->elevated);
          highP = p1;
          startIndex2 = index + 1;
        }
      }
      if (p != highP)
      {
        ...
        //     strncmp(p->name, "sh", sizeof("sh")) != 0)
        //   cprintf("RR picked %s\n", p->name);
        // if (strncmp(highP->name, "initcode", sizeof("initcode")) != 0 &&
        //     strncmp(highP->name, "init", sizeof("init")) != 0 &&
        //     strncmp(highP->name, "sh", sizeof("sh")) != 0)
        // {
        //   /* code */
        //   cprintf("nice picked %s\n", highP->name);
        // }
      }
